...
jest.mock('axios');
jest.mock('../../actions/pokemonActions');
...
describe('Api Response', () => {
  it('happy path -> returns a list of pokemon', async () => {
    ...
    axios.get = jest.fn().mockResolvedValue({ pokemons });
    const pokemonList = await axios.get('normal');
    expect(pokemonList).toEqual({ pokemons });
  });

  it('unhappy path -> returns error when type not found', async () => {
    const error = 'Not Found';
    axios.get = jest.fn().mockResolvedValue({ error });
    const pokemonListError = await axios.get('n');
  ...
  it('happy path -> returns single pokemon object', async () => {
    ...
    const pokemonObject = await axios.get('squirtle');
  ...
  it('unhappy path -> returns error when pokemon not found by name', async () => {
    ...
    axios.get = jest.fn().mockResolvedValue({ error });
    const pokemonError = await axios.get('erorororo');
